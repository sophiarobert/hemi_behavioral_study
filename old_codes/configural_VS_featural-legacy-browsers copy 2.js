...
function experimentInit() {
  ...
  fix_change_resp = new core.Keyboard({psychoJS: psychoJS, clock: new util.Clock(), waitForStart: true});
...
function exp_code_setupRoutineBegin(snapshot) {
  return function () {
    ...
    exp_code_setupComponents.forEach( function(thisComponent) {
...
function block_instructionRoutineBegin(snapshot) {
  return function () {
    ...
    block_instructionComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
...
function trialRoutineBegin(snapshot) {
  return function () {
    ...
    _key_resp_allKeys = [];
    fix_change_resp.keys = undefined;
    fix_change_resp.rt = undefined;
    _fix_change_resp_allKeys = [];
    // keep track of which components have finished
    trialComponents = [];
    ...
    trialComponents.push(fix_change_resp);
...
function trialRoutineEachFrame(snapshot) {
  return function () {
    ...
    // *fix_change_resp* updates
    if (t >= 0 && fix_change_resp.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      fix_change_resp.tStart = t;  // (not accounting for frame time here)
      fix_change_resp.frameNStart = frameN;  // exact frame index
      
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function() { fix_change_resp.clock.reset(); });  // t=0 on next screen flip
      psychoJS.window.callOnFlip(function() { fix_change_resp.start(); }); // start on screen flip
      psychoJS.window.callOnFlip(function() { fix_change_resp.clearEvents(); });
    }

    if (fix_change_resp.status === PsychoJS.Status.STARTED) {
      let theseKeys = fix_change_resp.getKeys({keyList: ['space'], waitRelease: false});
      _fix_change_resp_allKeys = _fix_change_resp_allKeys.concat(theseKeys);
      if (_fix_change_resp_allKeys.length > 0) {
        fix_change_resp.keys = _fix_change_resp_allKeys[_fix_change_resp_allKeys.length - 1].name;  // just the last key pressed
        fix_change_resp.rt = _fix_change_resp_allKeys[_fix_change_resp_allKeys.length - 1].rt;
        // a response ends the routine
        continueRoutine = false;
      }
    }
...
function trialRoutineEnd(snapshot) {
  return function () {
    ...
    key_resp.stop();
    psychoJS.experiment.addData('fix_change_resp.keys', fix_change_resp.keys);
    if (typeof fix_change_resp.keys !== 'undefined') {  // we had a response
        psychoJS.experiment.addData('fix_change_resp.rt', fix_change_resp.rt);
        routineTimer.reset();
        }
    
    fix_change_resp.stop();
