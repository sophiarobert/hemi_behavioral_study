...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/sophia/Documents/PycharmProjects/Local_VS_Global/config_feat_task.py',
...
for thisRun in runs:
    ...
    for thisComponent in start_face_instructionsComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    runs.addData('face_instructions.started', face_instructions.tStartRefresh)
    runs.addData('face_instructions.stopped', face_instructions.tStopRefresh)
    # check responses
    if start_exp_press.keys in ['', [], None]:  # No response was made
        start_exp_press.keys = None
    runs.addData('start_exp_press.keys',start_exp_press.keys)
    if start_exp_press.keys != None:  # we had a response
        runs.addData('start_exp_press.rt', start_exp_press.rt)
    runs.addData('start_exp_press.started', start_exp_press.tStartRefresh)
    runs.addData('start_exp_press.stopped', start_exp_press.tStopRefresh)
    ...
    for thisFeat_face_loop in feat_face_loop:
        ...
        if trial_order[trialNum] == 1:
            target_image1.setImage(face_feat_paths[trialSame[sameTrialid]])
            probe_image1.setImage(face_feat_paths[trialSame[sameTrialid]])
            sameTrialid += 1
        elif trial_order[trialNum] == 0:
            img_pair = trialDiff[diffTrial[diffTrialid]]
            target_image1.setImage(face_feat_paths[img_pair[0]])
            probe_image1.setImage(face_feat_paths[img_pair[1]])
            diffTrialid += 1
        else:
            print(trial_order[trialNum])

        # keep track of which components have finished
        feat_face_trialsComponents = [target_image1, ISI_1, probe_image1, key_resp]
        ...
        while continueRoutine:
            ...
            if key_resp.status == STARTED and not waitOnFlip:
                theseKeys = key_resp.getKeys(keyList=['d', 's'], waitRelease=False)
                ...
                if len(_key_resp_allKeys):
                    ...
                    continueRoutine = False
            # *ISI_1* period
            if ISI_1.status == NOT_STARTED and t >= 0.2-frameTolerance:
                # keep track of start time/frame for later
        ...
        feat_face_loop.addData('ISI_1.started', ISI_1.tStart)
        feat_face_loop.addData('ISI_1.stopped', ISI_1.tStop)
        ...
        feat_face_loop.addData('key_resp.keys',key_resp.keys)
        feat_face_loop.addData('key_resp.corr', key_resp.corr)
        if key_resp.keys != None:  # we had a response
            feat_face_loop.addData('key_resp.rt', key_resp.rt)
        feat_face_loop.addData('key_resp.started', key_resp.tStartRefresh)
        feat_face_loop.addData('key_resp.stopped', key_resp.tStopRefresh)
    ...
    while continueRoutine:
        ...
        if start_run2.status == STARTED and not waitOnFlip:
            theseKeys = start_run2.getKeys(keyList=['y', 'n', 'left', 'right', 'space'], waitRelease=False)
    ...
    for thisComponent in start_haus_instructionsComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    runs.addData('haus_instructions.started', haus_instructions.tStartRefresh)
    runs.addData('haus_instructions.stopped', haus_instructions.tStopRefresh)
    # check responses
    if start_run2.keys in ['', [], None]:  # No response was made
        start_run2.keys = None
    runs.addData('start_run2.keys',start_run2.keys)
    if start_run2.keys != None:  # we had a response
        runs.addData('start_run2.rt', start_run2.rt)
    runs.addData('start_run2.started', start_run2.tStartRefresh)
    runs.addData('start_run2.stopped', start_run2.tStopRefresh)
    ...
    for thisConf_haus_loop in conf_haus_loop:
        ...
        if trial_order[trialNum] == 1:
            target_image2.setImage(haus_config_paths[trialSame[sameTrialid]])
            probe_image2.setImage(haus_config_paths[trialSame[sameTrialid]])
            sameTrialid += 1
        elif trial_order[trialNum] == 0:
            img_pair = trialDiff[diffTrial[diffTrialid]]
            target_image2.setImage(haus_config_paths[img_pair[0]])
            probe_image2.setImage(haus_config_paths[img_pair[1]])
            diffTrialid += 1
        else:
            print('error:' + trial_order[trialNum])

        # keep track of which components have finished
        config_haus_trialsComponents = [target_image2, probe_image2, ISI_2, key_resp]
        ...
        conf_haus_loop.addData('key_resp.keys',key_resp.keys)
        conf_haus_loop.addData('key_resp.corr', key_resp.corr)
        if key_resp.keys != None:  # we had a response
            conf_haus_loop.addData('key_resp.rt', key_resp.rt)
        conf_haus_loop.addData('key_resp.started', key_resp.tStartRefresh)
        conf_haus_loop.addData('key_resp.stopped', key_resp.tStopRefresh)
    ...
    for thisConf_face_loop in conf_face_loop:
        ...
        if trial_order[trialNum] == 1:
            target_image3.setImage(face_config_paths[trialSame[sameTrialid]])
            probe_image3.setImage(face_config_paths[trialSame[sameTrialid]])
            sameTrialid += 1
        elif trial_order[trialNum] == 0:
            img_pair = trialDiff[diffTrial[diffTrialid]]
            target_image3.setImage(face_config_paths[img_pair[0]])
            probe_image3.setImage(face_config_paths[img_pair[1]])
            diffTrialid += 1

        # keep track of which components have finished
        config_face_trialsComponents = [target_image3, probe_image3, ISI_3, key_resp]
        ...
        while continueRoutine:
            ...
            if key_resp.status == STARTED and not waitOnFlip:
                ...
                if len(_key_resp_allKeys):
                    ...
                    continueRoutine = False
            # *ISI_3* period
            if ISI_3.status == NOT_STARTED and t >= 0.2-frameTolerance:
                # keep track of start time/frame for later
        ...
        conf_face_loop.addData('ISI_3.started', ISI_3.tStart)
        conf_face_loop.addData('ISI_3.stopped', ISI_3.tStop)
        # check responses
        if key_resp.keys in ['', [], None]:  # No response was made
            key_resp.keys = None
            # was no response the correct answer?!
        ...
        conf_face_loop.addData('key_resp.keys',key_resp.keys)
        conf_face_loop.addData('key_resp.corr', key_resp.corr)
        if key_resp.keys != None:  # we had a response
            conf_face_loop.addData('key_resp.rt', key_resp.rt)
        conf_face_loop.addData('key_resp.started', key_resp.tStartRefresh)
        conf_face_loop.addData('key_resp.stopped', key_resp.tStopRefresh)
    ...
    for thisFeat_haus_loop in feat_haus_loop:
        ...
        if trial_order[trialNum] == 1:
            target_image4.setImage(haus_feat_paths[trialSame[sameTrialid]])
            probe_image4.setImage(haus_feat_paths[trialSame[sameTrialid]])
            sameTrialid += 1
        elif trial_order[trialNum] == 0:
            img_pair = trialDiff[diffTrial[diffTrialid]]
            target_image4.setImage(haus_feat_paths[img_pair[0]])
            probe_image4.setImage(haus_feat_paths[img_pair[1]])
            diffTrialid += 1
        else:
            print(trial_order[trialNum])

        # keep track of which components have finished
        feat_haus_trialsComponents = [target_image4, probe_image4, ISI_4, key_resp]
        ...
        while continueRoutine:
            ...
            if key_resp.status == STARTED and not waitOnFlip:
                ...
                if len(_key_resp_allKeys):
                    ...
                    continueRoutine = False
            # *ISI_4* period
            if ISI_4.status == NOT_STARTED and t >= 0.2-frameTolerance:
                # keep track of start time/frame for later
        ...
        for thisComponent in feat_haus_trialsComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        feat_haus_loop.addData('target_image.started', target_image.tStartRefresh)
        feat_haus_loop.addData('target_image.stopped', target_image.tStopRefresh)
        feat_haus_loop.addData('probe_image.started', probe_image.tStartRefresh)
        feat_haus_loop.addData('probe_image.stopped', probe_image.tStopRefresh)
        feat_haus_loop.addData('ISI_4.started', ISI_4.tStart)
        feat_haus_loop.addData('ISI_4.stopped', ISI_4.tStop)
        # check responses
        if key_resp.keys in ['', [], None]:  # No response was made
            key_resp.keys = None
            # was no response the correct answer?!
            if str(corrAns).lower() == corrAns:
               key_resp.corr = 1;  # correct non-response
            else:
               key_resp.corr = 0;  # failed to respond (incorrectly)
        # store data for feat_haus_loop (TrialHandler)
        feat_haus_loop.addData('key_resp.keys',key_resp.keys)
        feat_haus_loop.addData('key_resp.corr', key_resp.corr)
        if key_resp.keys != None:  # we had a response
            feat_haus_loop.addData('key_resp.rt', key_resp.rt)
        feat_haus_loop.addData('key_resp.started', key_resp.tStartRefresh)
        feat_haus_loop.addData('key_resp.stopped', key_resp.tStopRefresh)
